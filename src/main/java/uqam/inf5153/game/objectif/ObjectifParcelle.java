...
public class ObjectifParcelle extends Objectif {
    ...
    public int appliquerObjectif(PlateauDeJeu plateau){
        switch (identifiant){
            case 1:
                for (ComposantParcelle parcelle : plateau.getParcelles()) {
                    if (parcelle.getComposant().getCouleur()== Couleur.VERT){
                        for (int i=0; i<6; i++){
                            Parcelle[] pAdj=parcelle.getComposant().parcellesAdjacentes();
            ...
            case 3:
                for (ComposantParcelle parcelle : plateau.getParcelles()) {
                    if (parcelle.getComposant().getCouleur()==Couleur.ROSE){
                        for (int i=0; i<6; i++){
                            Parcelle[] pAdj=parcelle.getComposant().parcellesAdjacentes();
            ...
            case 6:
                for (ComposantParcelle parcelle : plateau.getParcelles()) {
                    if (parcelle.getComposant().getCouleur()==Couleur.JAUNE){
                        for (int i=0; i<6; i++){
                            Parcelle[] pAdj=parcelle.getComposant().parcellesAdjacentes();
